...
class MADDPGMix:
    ...
    def __init__(self, env : DeepNav, smarties=1, n_epochs=1000, n_episodes=10, tau=0.005,
        ...
        self.smarties = np.random.randint(0, self.n_agents, smarties).tolist()
        ...
        for agnt in range(self.n_agents):
            if agnt in self.smarties:
                ...
                self.agents.append(ORCAAgent(agnt, self.env))
    ...
    def updateTarget(self):
        ...
        for i in self.smarties:
            self.agents[i]['target_a_n'].set_weights(self.agents[i]['a_n'].weights)
            self.agents[i]['target_q_n'].set_weights(self.agents[i]['q_n'].weights) 

    def save(self):
        for i in self.smarties:
            _id = self.agents[i]['id']
            self.agents[i]['q_n'].save(self.path + f'QNet_{_id}.h5')
            self.agents[i]['target_q_n'].save(self.path + f'QTargetNet_{_id}.h5')
        
            self.agents[i]['a_n'].save(self.path + f'ANet_{_id}.h5')
            self.agents[i]['target_a_n'].save(self.path + f'ATargetNet_{_id}.h5')
            
    def load(self):
        for i in self.agents:
            _id = self.agents[i]['id']
            self.agents[i]['q_n'] = keras.models.load_model(self.path + f'QNet_{_id}.h5')
            self.agents[i]['target_q_n'] = keras.models.load_model(self.path + f'QTargetNet_{_id}.h5')

            self.agents[i]['a_n'] = keras.models.load_model(self.path + f'ANet_{_id}.h5')
            self.agents[i]['target_a_n'] = keras.models.load_model(self.path + f'ATargetNet_{_id}.h5')
        ...
        for i in range(self.n_epochs):
            for j in range(self.n_episodes):
                ...
                while 1:
                    ...
                    if self.rb.ready:
                        q_l, a_l = self._learn(self.rb.sample())
                        ql.append(q_l)
                        al.append(a_l)
                        self.updateTarget()
                    s = s_1
    ...
    def _learn(self, sampledBatch):
        ...
        for i in self.smarties:
            ...
            with tf.GradientTape(True) as tape:
                ...
                a_loss = tf.math.reduce_mean(q_values)
            ...
            actorGrad = tape.gradient(a_loss, agnt['a_n'].trainable_variables)
